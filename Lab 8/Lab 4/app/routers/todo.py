...
@todo_router.post('/todos/{id}/category/{cat_id}', status_code=status.HTTP_200_OK)
def assign_category_to_todo(id:int, cat_id:int, db:SessionDep, user:AuthDep):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="An error occurred while assigning category to item",
        )
